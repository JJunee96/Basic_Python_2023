...
class Contact:
    ...
    # 12. 각 멤버변수 접근하는 함수            
    # getName, getPhone_num, getEmail, getAddr
...
def save_contacts(list):
    file = open('C:/Source/Study_Python2023/Project/contacts.txt', 'w', encoding = 'utf-8')

    for item in list:
        text = f'{item.getName()}/{item.getPhoneNum()}/{item.getEmail()}/{item.getAddr()}'
        file.write(f'{text}\n')
    
    file.close() # 파일은 마지막에 닫아줘야함!

# 14. 주소록 읽어오기
def load_contacts(list):
    try:
        file = open('C:/Source/Study_Python2023/Project/contacts.txt', 'r', encoding = 'utf-8')
    except Exception as e: # 15-1. 예외처리
        f = open('C:/Source/Study_Python2023/Project/contacts.txt', 'w', encoding = 'utf-8')
...
def run():
    ...
    # temp = Contact('조원준', '010-9906-1847', 'wnsltkak@naver.com', '수영구 광안동...')
    # set_contact()
    clear_console()
    while True:
        sel_menu = get_menu()
